...
public class FurnaceRecipeRender extends AbstractRecipeRender<AbstractCookingRecipe> {
    ...
    public void render(GuiGraphics pGraphics, RegistryAccess pAccess, int pMouseX, int pMouseY, IModScreen pScreen, Font pFont, IBook pBook, ItemRotation pRotation) {
        ...
        RenderUtils.renderImage(pGraphics, FURNACE_GRID, pScreen.xOffset() + pScreen.screenWidth() / 2 - 57, pScreen.yOffset() + pScreen.screenHeight() / 2 - 63,  57, 63);

        int x = pScreen.xOffset() + pScreen.screenWidth() / 2 - 52;
        int y = pScreen.yOffset() + pScreen.screenHeight() / 2 - 58;
        ...
        int outputX = pScreen.xOffset() + pScreen.screenWidth() / 2 + 39;
        int outputY = pScreen.yOffset() + pScreen.screenHeight() / 2 + 24;
        ...
        int fuelX = pScreen.xOffset() + pScreen.screenWidth() / 2 - 52;
        int fuelY = pScreen.yOffset() + pScreen.screenHeight() / 2 + 25;
